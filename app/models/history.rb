class History < ActiveRecord::Base
  ...
  scope :notification, -> (project_id, user_id) { joins("LEFT JOIN notification_subcriptions ON notification_subcriptions.history_id = histories.id " + " INNER JOIN tasks ON tasks.id = histories.task_id " +
                                                   "INNER JOIN projects ON projects.id = tasks.project_id
                                                    where tasks.project_id = #{project_id} and ( notification_subcriptions.user_id = #{user_id} or notification_subcriptions.user_id is NULL ) order by histories.created_at DESC") }
