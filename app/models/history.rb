class History < ActiveRecord::Base
  ...
  scope :feeds, -> (user_id) { joins("INNER JOIN tasks ON tasks.id = histories.task_id " +
                                         "INNER JOIN projects ON projects.id = tasks.project_id INNER JOIN projects_users ON projects_users.project_id = projects.id where projects_users.user_id = #{user_id} order by projects.created_at DESC limit 15") }
end
